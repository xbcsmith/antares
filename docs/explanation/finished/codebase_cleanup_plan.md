...
- [x] Error-swallowing `let _ =` discards logged (2 genuine `Result` sites
      previously fixed; the 4 remaining `Option`/struct sites — `inventory_ui.rs`
      `None` cases and `spell_casting.rs` `UtilityResult` discard — now also
      being addressed)
...
- [x] Shared UI palette + `title_bar_with_hints` + `format_gold`
- [x] Column-layout helper adopted in 3+ screens
- [x] `sdk/database.rs` routed through `impl_ron_database!`
- [x] `campaign_loader` `load_optional_ron` collapse
- [x] Combat cleanup helpers (`despawn_all`, reset)
- [ ] `GameMode` run-condition helper (being implemented now)
- [x] Inventory UI convergence
- [ ] `too_many_arguments` UI/combat helpers refactored (being implemented now)
...
- [x] Reputation/faction system implemented (state + 3 call sites)
- [x] Jump spell targeting implemented (SP charged only on success)
- [x] Quest `SetFlag` persistence implemented
- [x] `CameraMode::Tactical` + `::Isometric` implemented
- [x] Skill-rank temporary modifiers implemented
- [x] Recruitment confirmation UI implemented
- [x] Monster visuals + `CreatureAnimationState` implemented
...
> **Status**: All Phase 6 criteria are met. All seven features are implemented
> and tested; all four quality gates (`fmt`, `check`, `clippy`, `nextest`) pass
> with zero warnings.
