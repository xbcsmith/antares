...
I want some type of autocomplete system for entering values. For example, if I am entering a number, I want to see what the valid range is. If I am entering a string, I want to see if there are any restrictions on the string (like length or allowed characters). Example when entering a group of monsters, I want to see what monsters are available to choose from. I don't want a bunch of labels I want to start typing and see a list of options.

Library to use: egui_autocomplete

https://crates.io/crates/egui_autocomplete

### Trap Events

- Trap Effects should match up to conditions.ron. So we should look up the condition list and link the condition to the effect. Damage should be a number of HPs not a string.
