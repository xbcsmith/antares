...
- [ ] `advanced_trees.rs` branch mesh no longer includes leaf/foliage sphere geometry.
- [ ] Bark mesh UVs exist and are suitable for texture repetition.
- [ ] Tree foliage transforms align with trunk transforms.
- [ ] `foliage_density` visibly affects tree foliage.
- [ ] Dead trees never spawn foliage.
- [ ] Shrubs no longer clip trunk centers by default.
- [ ] SDK metadata edits produce visible runtime changes.
- [ ] Regression tests cover the fixed behavior.
