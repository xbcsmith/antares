...
- Foliage silhouettes are generated by species-shaped geometry, not round alpha-mask textures.
- Foliage materials are opaque, unlit, species-colored, and double-sided.
...
| LOD2   | Lowest-detail branch mesh silhouette with reduced or suppressed foliage  |
...
The map spawn path creates LOD0, LOD1, and LOD2 child meshes up front. LOD2 deliberately uses simplified branch geometry rather than an opaque billboard slab, because untextured or bark-textured impostor quads read as giant black rectangles in first-person views. The LOD switching system shows only the matching child mesh group for the active camera-distance bucket.
...
- shared material variants keyed by tint, color variation, and material budget
...
3. Current camera distance is not selecting LOD2, which intentionally uses reduced foliage.
