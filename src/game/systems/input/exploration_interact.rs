...
mod tests {
    ...
    use crate::domain::world::{LockState, Map};
    ...
    #[test]
    fn test_try_interact_plain_tile_door_returns_false_when_no_plain_door_exists() {
        let mut game_state = build_game_state();
    ...
    fn test_try_interact_locked_door_event_consumes_key_and_opens_tile() {
        ...
        assert!(lock_pending.lock_id.is_none());
    }
    ...
    #[test]
    fn test_populate_lock_pending_sets_lockpick_flag_from_party_abilities() {
        let mut game_state = build_game_state();
        game_state.party.members[0].class_id = "thief".to_string();

        let mut db = crate::sdk::database::ContentDatabase::new();
        let mut thief_class =
            crate::domain::classes::ClassDefinition::new("thief".to_string(), "Thief".to_string());
        thief_class.special_abilities.push("pick_lock".to_string());
        db.classes.add_class(thief_class).unwrap();
        let content = GameContent::new(db);

        let mut lock_pending = LockInteractionPending::default();

        populate_lock_pending(
            &game_state,
            Some(&content),
            &mut lock_pending,
            DOOR_LOCK_ID.to_string(),
            Position::new(5, 4),
        );
