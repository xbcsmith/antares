...
impl ProceduralMeshCache {
    ...
    ///
    /// Each `TreeType` variant gets its own lit, detail-textured foliage material.
    /// The texture is loaded via `asset_server` and the material uses
    /// `AlphaMode::Mask` for cutout transparency. `double_sided` and
    /// `cull_mode: None` ensure generated leaves and fronds are visible from
    /// both sides.
    ///
    /// # Arguments
    ///
    /// * `tree_type` - Determines the species colour and detail texture path
    /// * `asset_server` - Used to load the foliage detail texture
    /// * `materials` - Mutable reference to the material asset storage
    ///
    /// # Returns
    ///
    ...
    pub fn get_or_create_foliage_material(
        ...
        if let Some(handle) = self.tree_foliage_materials.get(&tree_type) {
            ...
            tracing::debug!(
                tree_type = ?tree_type,
                "creating lit detail-textured foliage material for {tree_type:?}"
            );
            let texture =
                super::creature_meshes::load_texture(asset_server, foliage_texture_path(tree_type));
            let handle = materials.add(StandardMaterial {
    ...
    fn get_or_create_foliage_material_variant(
        ...
        // Variant inherits the detail texture and AlphaMode::Mask from the base material.
        // Apply the variant colour as a multiply tint; the light-neutral detail texture
        // preserves full species/tile colour fidelity.
        material.base_color = color;
...
/// Asset path for the Birch foliage detail texture used by the lit foliage material.
const TREE_FOLIAGE_TEXTURE_BIRCH: &str = "assets/textures/trees/foliage_birch.png";
/// Asset path for the Willow foliage detail texture used by the lit foliage material.
const TREE_FOLIAGE_TEXTURE_WILLOW: &str = "assets/textures/trees/foliage_willow.png";
/// Asset path for the Palm foliage detail texture used by the lit foliage material.
const TREE_FOLIAGE_TEXTURE_PALM: &str = "assets/textures/trees/foliage_palm.png";
/// Asset path for the Shrub foliage detail texture used by the lit foliage material.
const TREE_FOLIAGE_TEXTURE_SHRUB: &str = "assets/textures/trees/foliage_shrub.png";
/// Alpha cutoff used for foliage detail texture masking (`AlphaMode::Mask`).
...
/// Returns the foliage detail texture path for the given tree type.
///
/// Used by `get_or_create_foliage_material` to load a per-species detail texture
/// for the lit, `AlphaMode::Mask` foliage material. `Dead` trees have zero
/// foliage density, so their path falls back to Oak.
...
/// * `asset_server` - Asset server used to resolve bark textures; foliage uses generated materials
/// * `position` - Tile position in world coordinates
/// * `map_id` - Map identifier for cleanup
...
mod tests {
    ...
    /// Tests that foliage materials use species-specific base colours, load a detail texture, and are lit.
    #[test]
    fn test_foliage_material_uses_species_color_detail_texture_and_is_lit() {
        fn create_foliage_material_system(
            ...
            assert!(
                oak_material.base_color_texture.is_some(),
                "Oak foliage material must load the detail texture"
            );
            assert!(
                pine_material.base_color_texture.is_some(),
                "Pine foliage material must load the detail texture"
            );
            assert_eq!(
            ...
            assert!(
                ...
                "Foliage materials must be lit"
            );
            assert!(
                matches!(oak_material.alpha_mode, AlphaMode::Mask(_)),
                "Foliage material must use AlphaMode::Mask for detail texture cutout"
            );
            assert!(
    ...
    /// Tests that tinted foliage material variants are cached, species-specific, and lit.
    #[test]
    fn test_foliage_material_variant_cache_reuses_species_tint_bucket_and_is_lit() {
        fn create_foliage_variants_system(
            ...
            assert!(
                !material.unlit,
                "Cached foliage material variants must be lit"
            );
            assert!(
                material.base_color_texture.is_some(),
                "Cached foliage material variants must preserve the detail texture"
            );
            assert!(
                matches!(material.alpha_mode, AlphaMode::Mask(_)),
                "Cached foliage material variants must use AlphaMode::Mask for detail texture cutout"
            );
            assert!(
