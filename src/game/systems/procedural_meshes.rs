...
impl ProceduralMeshCache {
    ...
    /// # Arguments
    ///
    /// * `tree_type` - Determines the generated species colour
    /// * `asset_server` - Retained for API compatibility; foliage textures are not loaded here
    /// * `materials` - Mutable reference to the material asset storage
    ///
    /// # Returns
    ///
    ...
    pub fn get_or_create_foliage_material(
        ...
        if let Some(handle) = self.tree_foliage_materials.get(&tree_type) {
            ...
            tracing::debug!(
                tree_type = ?tree_type,
                "creating generated procedural tree foliage material; legacy foliage texture path is diagnostic-only"
            );
            let handle = materials.add(StandardMaterial {
...
/// Deprecated test-only asset path for the Oak foliage texture.
///
/// Imported landscape definitions are preferred for authored trees. Procedural
/// fallback foliage uses generated geometry and species colours, not this texture.
#[cfg(test)]
const TREE_FOLIAGE_TEXTURE_OAK: &str = "assets/textures/trees/foliage_oak.png";
/// Deprecated test-only asset path for the Pine foliage texture.
///
/// Imported landscape definitions are preferred for authored trees. Procedural
/// fallback foliage uses generated geometry and species colours, not this texture.
#[cfg(test)]
const TREE_FOLIAGE_TEXTURE_PINE: &str = "assets/textures/trees/foliage_pine.png";
/// Deprecated test-only asset path for the Birch foliage texture.
///
/// Imported landscape definitions are preferred for authored trees. Procedural
/// fallback foliage uses generated geometry and species colours, not this texture.
#[cfg(test)]
const TREE_FOLIAGE_TEXTURE_BIRCH: &str = "assets/textures/trees/foliage_birch.png";
/// Deprecated test-only asset path for the Willow foliage texture.
///
/// Imported landscape definitions are preferred for authored trees. Procedural
/// fallback foliage uses generated geometry and species colours, not this texture.
#[cfg(test)]
const TREE_FOLIAGE_TEXTURE_WILLOW: &str = "assets/textures/trees/foliage_willow.png";
/// Deprecated test-only asset path for the Palm foliage texture.
///
/// Imported landscape definitions are preferred for authored trees. Procedural
/// fallback foliage uses generated geometry and species colours, not this texture.
#[cfg(test)]
const TREE_FOLIAGE_TEXTURE_PALM: &str = "assets/textures/trees/foliage_palm.png";
/// Deprecated test-only asset path for the Shrub foliage texture.
///
/// Imported landscape definitions are preferred for authored brush. Procedural
/// fallback foliage uses generated geometry and species colours, not this texture.
#[cfg(test)]
...
/// Returns the deprecated test-only foliage texture path for the given tree type.
///
...
/// tests so fixture completeness checks can refer to the same canonical asset
/// names. `Dead` trees have zero foliage density, so their path is diagnostic
/// only and falls back to Oak.
#[cfg(test)]
