...
/// Geometry descriptor for a single rotated-ellipse stamp.
///
/// Bundles the five position/shape parameters into one struct so that
/// [`draw_ellipse`] stays under the Clippy `too_many_arguments` threshold.
pub struct EllipseGeometry {
    /// Centre X coordinate (may be negative; wraps modulo 512).
    pub cx: i32,
    /// Centre Y coordinate (may be negative; wraps modulo 512).
    pub cy: i32,
    /// Semi-axis along the local X direction (px).
    pub semi_a: f32,
    /// Semi-axis along the local Y direction (px).
    pub semi_b: f32,
...
/// * `geom`  – Ellipse geometry (centre, semi-axes, rotation).
...
pub fn draw_ellipse(
    ...
    let (sin_a, cos_a) = geom.angle.sin_cos();
    // A rotated ellipse can extend to max(semi_a, semi_b) in both axes.
    let bound = geom.semi_a.max(geom.semi_b).ceil() as i32 + 1;
    ...
    for dy in -bound..=bound {
        for dx in -bound..=bound {
            ...
            let d2 = (rx / geom.semi_a).powi(2) + (ry / geom.semi_b).powi(2);
            if d2 <= threshold {
                ...
                let wx = (geom.cx + dx).rem_euclid(512) as u32;
                let wy = (geom.cy + dy).rem_euclid(512) as u32;
