...
impl TreeTextureValidationIssue {
    /// Creates a missing-file validation issue.
    fn missing(spec: TreeTextureSpec) -> Self {
        Self {
    ...
    fn dimension_mismatch(spec: TreeTextureSpec, actual_dimensions: (u32, u32)) -> Self {
        Self {
            validation_id: spec.validation_id,
            expected_path: PathBuf::from(spec.relative_path),
    ...
    fn misnamed(
        ...
        Self {
            validation_id: "SDK-TEX-02",
            expected_path: PathBuf::from(spec.relative_path),
            actual_path: Some(actual_path.clone()),
            expected_dimensions: (spec.expected_width, spec.expected_height),
            actual_dimensions,
            message: format!(
                "SDK-TEX-02: expected exact filename '{}' with dimensions {}x{}, but found misnamed asset '{}' with dimensions {}",
                spec.relative_path,
                spec.expected_width,
                spec.expected_height,
                actual_path.display(),
...
pub const fn required_tree_texture_specs() -> [TreeTextureSpec; 7] {
    [
        TreeTextureSpec {
            validation_id: "SDK-TEX-03",
            relative_path: "assets/textures/trees/bark.png",
            expected_width: 64,
            expected_height: 128,
        },
        TreeTextureSpec {
            validation_id: "SDK-TEX-04",
            relative_path: "assets/textures/trees/foliage_oak.png",
            expected_width: 128,
            expected_height: 128,
        },
        TreeTextureSpec {
            validation_id: "SDK-TEX-05",
            relative_path: "assets/textures/trees/foliage_pine.png",
            expected_width: 64,
            expected_height: 128,
        },
        TreeTextureSpec {
            validation_id: "SDK-TEX-06",
            relative_path: "assets/textures/trees/foliage_birch.png",
            expected_width: 128,
            expected_height: 128,
        },
        TreeTextureSpec {
            validation_id: "SDK-TEX-07",
            relative_path: "assets/textures/trees/foliage_willow.png",
            expected_width: 128,
            expected_height: 128,
        },
        TreeTextureSpec {
            validation_id: "SDK-TEX-08",
            relative_path: "assets/textures/trees/foliage_palm.png",
            expected_width: 128,
            expected_height: 128,
        },
        TreeTextureSpec {
            validation_id: "SDK-TEX-09",
            relative_path: "assets/textures/trees/foliage_shrub.png",
            expected_width: 64,
            expected_height: 64,
        },
    ]
...
impl AssetManager {
    ...
    /// Validates the required runtime terrain tree textures for the current campaign.
    ///
    /// This checks exact required filenames and exact image dimensions for the
    /// runtime-generated tree textures expected by the terrain system.
    pub fn validate_tree_texture_assets(&self) -> Vec<TreeTextureValidationIssue> {
        ...
        for spec in required_specs {
            ...
            if !full_expected_path.exists() {
                issues.push(TreeTextureValidationIssue::missing(spec));
            } else {
                match image::open(&full_expected_path) {
                    Ok(image) => {
                        ...
                        if actual_dimensions != (spec.expected_width, spec.expected_height) {
                            issues.push(TreeTextureValidationIssue::dimension_mismatch(
                                spec,
                                actual_dimensions,
                            ));
                        }
                    }
                    Err(error) => issues.push(TreeTextureValidationIssue::unreadable(
                        spec,
                        expected_path.clone(),
                        &error,
                    )),
            ...
            let prefix = if stem.starts_with("foliage_") {
                "foliage_"
            } else {
                stem
            };
            ...
            for asset_path in self.assets.keys() {
                ...
                let parent_matches = asset_path.parent() == Some(tree_texture_dir.as_path());
                if !parent_matches {
                    continue;
                }
                ...
                let is_misnamed_variant = if prefix == "foliage_" {
                    asset_stem.starts_with(prefix)
                        && asset_extension.eq_ignore_ascii_case(extension)
                } else {
                    asset_stem == stem && asset_extension.eq_ignore_ascii_case(extension)
                };
                ...
                issues.push(TreeTextureValidationIssue::misnamed(
                    spec,
                    asset_path.clone(),
                    dimensions,
                ));
...
mod tests {
    ...
    fn tree_texture_test_dir(name: &str) -> PathBuf {
    ...
    #[test]
    fn test_validate_tree_texture_assets_reports_missing_required_file() {
        let tmp_dir = tree_texture_test_dir("test_validate_tree_texture_assets_missing");
        std::fs::create_dir_all(&tmp_dir).unwrap();
        create_valid_tree_texture_set(&tmp_dir);
        std::fs::remove_file(tmp_dir.join("assets/textures/trees/bark.png")).unwrap();
        ...
        assert!(issues.iter().any(|issue| {
            issue.validation_id == "SDK-TEX-03"
                ...
                && issue
                    .message
                    .contains("expected filename 'assets/textures/trees/bark.png'")
                && issue.message.contains("64x128")
    ...
    #[test]
    fn test_validate_tree_texture_assets_reports_misnamed_tree_texture_file() {
        let tmp_dir = tree_texture_test_dir("test_validate_tree_texture_assets_misnamed");
        std::fs::create_dir_all(&tmp_dir).unwrap();
        create_valid_tree_texture_set(&tmp_dir);
        std::fs::remove_file(tmp_dir.join("assets/textures/trees/foliage_oak.png")).unwrap();
        write_test_png(
            &tmp_dir.join("assets/textures/trees/foliage_oak_variant.png"),
            128,
            128,
        );
        ...
        assert!(issues.iter().any(|issue| {
            issue.validation_id == "SDK-TEX-02"
                ...
                && issue
                    .message
                    .contains("expected exact filename 'assets/textures/trees/foliage_oak.png'")
                && issue
                    .message
                    .contains("misnamed asset 'assets/textures/trees/foliage_oak_variant.png'")
        }));
    ...
    #[test]
    fn test_validate_tree_texture_assets_reports_bark_dimension_mismatch() {
        let tmp_dir = tree_texture_test_dir("test_validate_tree_texture_assets_bark_dimensions");
        std::fs::create_dir_all(&tmp_dir).unwrap();
        create_valid_tree_texture_set(&tmp_dir);
        write_test_png(&tmp_dir.join("assets/textures/trees/bark.png"), 63, 128);
    ...
    #[test]
    fn test_validate_tree_texture_assets_reports_pine_dimension_mismatch() {
        let tmp_dir = tree_texture_test_dir("test_validate_tree_texture_assets_pine_dimensions");
        std::fs::create_dir_all(&tmp_dir).unwrap();
        create_valid_tree_texture_set(&tmp_dir);
        write_test_png(
            &tmp_dir.join("assets/textures/trees/foliage_pine.png"),
            64,
            127,
        );
        ...
        assert!(issues.iter().any(|issue| {
            issue.validation_id == "SDK-TEX-05"
                ...
                && issue
                    .message
                    .contains("expected filename 'assets/textures/trees/foliage_pine.png'")
                && issue.message.contains("64x128")
                && issue.message.contains("64x127")
        }));
    ...
    #[test]
    fn test_validate_tree_texture_assets_accepts_fully_valid_required_tree_texture_set() {
        let tmp_dir = tree_texture_test_dir("test_validate_tree_texture_assets_valid");
        std::fs::create_dir_all(&tmp_dir).unwrap();
        create_valid_tree_texture_set(&tmp_dir);
