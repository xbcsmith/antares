...
impl CampaignMetadataEditBuffer {
    ...
    pub fn from_metadata(m: &crate::CampaignMetadata) -> Self {
        Self {
            ...
            skills_file: m.skills_file.clone(),
            ...
            landscape_file: m.landscape_file.clone(),
            terrain_file: m.terrain_file.clone(),
    ...
    pub fn apply_to(&self, dest: &mut crate::CampaignMetadata) {
        ...
        dest.skills_file = self.skills_file.clone();
        ...
        dest.landscape_file = self.landscape_file.clone();
        dest.terrain_file = self.terrain_file.clone();
                        ...
                        CampaignSection::Files => {
                            ...
                            egui::Grid::new("campaign_files_grid")
                                ...
                                .show(right_ui, |ui| {
                                    ...
                                    // Skills File
                                    ui.label("Skills File:");
                                    ui.horizontal(|ui| {
                                        if ui
                                            .text_edit_singleline(&mut self.buffer.skills_file)
                                            .changed()
                                        {
                                            self.has_unsaved_changes = true;
                                            *ctx.unsaved_changes = true;
                                        ...
                                        if ui.button("📁").on_hover_text("Browse").clicked() {
                                            if let Some(p) = rfd::FileDialog::new()
                                                ...
                                                .pick_file()
                                            {
                                                self.buffer.skills_file = p.display().to_string();
                                                self.has_unsaved_changes = true;
                                                *ctx.unsaved_changes = true;
                                            }
                                        }
                                    ...
                                    // Furniture File
                                    ui.label("Furniture File:");
                                    ui.horizontal(|ui| {
                                        if ui
                                            .text_edit_singleline(&mut self.buffer.furniture_file)
                                            .changed()
                                        {
                                            self.has_unsaved_changes = true;
                                            *ctx.unsaved_changes = true;
                                    ...
                                    // Landscape File
                                    ui.label("Landscape File:");
                                    ui.horizontal(|ui| {
                                        if ui
                                            .text_edit_singleline(&mut self.buffer.landscape_file)
                                            .changed()
                                        {
                                            self.has_unsaved_changes = true;
                                            *ctx.unsaved_changes = true;
                                    ...
                                    // Terrain File
                                    ui.label("Terrain File:");
                                    ui.horizontal(|ui| {
                                        if ui
                                            .text_edit_singleline(&mut self.buffer.terrain_file)
                                            .changed()
                                        {
                                            self.has_unsaved_changes = true;
                                            *ctx.unsaved_changes = true;
