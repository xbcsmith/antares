...
fn test_dialogue_state_start_initializes_fields() {
    ...
    let state = DialogueState::start(42, 1, None, None);
    assert!(state.is_active());
...
fn test_dialogue_state_end_clears_all_state() {
    ...
    let mut state = DialogueState::start(5, 1, None, None);
    state.update_node(
        "Some text".to_string(),
...
fn test_dialogue_state_multiple_node_chain() {
    ...
    let mut state = DialogueState::start(1, 10, None, None);
...
fn test_game_mode_dialogue_variant() {
    ...
    let state = DialogueState::start(1, 1, None, None);
