...
fn test_campaign_loads_creature_database() {
    ...
    let campaign_result = Campaign::load("campaigns/tutorial");
...
fn test_campaign_creature_database_contains_expected_creatures() {
    ...
    let content = ContentDatabase::load_campaign("campaigns/tutorial")
        .expect("Should load tutorial campaign content");

    // Assert: Database contains expected creature count (32 creatures in registry)
    assert_eq!(
        content.creatures.count(),
        32,
        "Tutorial campaign should have 32 registered creatures"
    );
...
fn test_campaign_integration_end_to_end() {
    ...
    let campaign = Campaign::load("campaigns/tutorial").expect("Tutorial campaign should load");
